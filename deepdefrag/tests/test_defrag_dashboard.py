...
from optical_rl_gym.envs.defragmentation_env import choose_randomly, OldestFirst, \
    assigning_path_without_defragmentation, HighMetricFirst, HighCutMetricFirst
...
def run_with_callback(callback, env_args_f, num_eps, log_dir_f, load_traffic):
    ...
    env_df = Monitor(env_df, log_dir_f + f"{load_traffic}",
                     info_keywords=('episode_service_blocking_rate', 'service_blocking_rate',
                                    ...
                                    'episode_service_blocking_rate_fragmentation', 'episode_service_blocking_rate_lack',
                                    'episode_cuts', 'episode_frag_metric', 'episode_shanon_entropy'))
...
def run_oldest(defrag_period, number_moves, env_args_f, num_eps, log_dir_f, fragmented_constraint, load_traffic):
    ...
    env_df_oldest = Monitor(env_df_oldest,
                            log_dir_f + f'df-oldest-{defrag_period}-{number_moves}-{fragmented_constraint}-{load_traffic}',
                            info_keywords=('episode_service_blocking_rate', 'service_blocking_rate',
                                           'reward', 'number_movements',
                                           'number_defragmentation_procedure', 'number_arrivals',
                                           'bit_rate_blocking_rate', 'number_movements_episode',
                                           ...
                                           'episode_service_blocking_rate_lack', 'episode_cuts', 'episode_frag_metric',
                                           'episode_shanon_entropy'))
    evaluate_heuristic(env_df_oldest, oldest_scenario.choose_oldest_first, n_eval_episodes=num_eps)

    return env_df_oldest


def run_highest(defrag_period, number_moves, env_args_f, num_eps, log_dir_f, fragmented_constraint, load_traffic, only_FF):
    highest_scenario = HighMetricFirst(defrag_period, number_moves)
    env_df_highest = gym.make('Defragmentation-v0', **env_args_f)
    env_df_highest = Monitor(env_df_highest,
                             log_dir_f + f'df-highest-{defrag_period}-{number_moves}-{fragmented_constraint}-{load_traffic}-{only_FF}',
                             info_keywords=('episode_service_blocking_rate', 'service_blocking_rate',
                                            'reward', 'number_movements',
                                            'number_defragmentation_procedure', 'number_arrivals',
                                            'bit_rate_blocking_rate', 'number_movements_episode',
...
def run_highest_cut(defrag_period, number_moves, env_args_f, num_eps, log_dir_f, fragmented_constraint, load_traffic, only_FF):
    highest_scenario = HighCutMetricFirst(defrag_period, number_moves)
    env_df_highest = gym.make('Defragmentation-v0', **env_args_f)
    env_df_highest = Monitor(env_df_highest,
                             log_dir_f + f'df-highest-cut-{defrag_period}-{number_moves}-{fragmented_constraint}-{load_traffic}-{only_FF}',
                             info_keywords=('episode_service_blocking_rate', 'service_blocking_rate',
                                            'reward', 'number_movements',
                                            'number_defragmentation_procedure', 'number_arrivals',
                                            'bit_rate_blocking_rate', 'number_movements_episode',
...
def test_dashboard():
    ...
    monitor_files = []
    policies = []
    logging_dir = "../examples/stable_baselines3/icton-heuristic-results-v1/"
    ...
    # topologies = ['nsfnet_chen_eon', 'Germany50']
    ...
    traffic_type = 1
    processes = []
    for topology_name, min_load, max_load, step_length in zip(topologies, min_loads, max_loads, step_lengths):
        print(os.getcwd())
        ...
        for load_counter, load_traffic in enumerate(range(min_load, max_load, step_length)):
            ...
            # p = Process(target=run_with_callback, args=(
            # assigning_path_without_defragmentation, copy.deepcopy(env_args), episodes, log_dir + 'df', load_traffic))
            # p.start()
            # processes.append(p)
            ...
            for i in [(10,10) ]:
                ...
                env_oldest = run_oldest(i[0], i[1], copy.deepcopy(env_args), episodes, log_dir, fragmented_constraint, load_traffic)
