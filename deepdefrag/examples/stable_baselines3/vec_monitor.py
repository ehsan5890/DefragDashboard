    ...
    def __init__(
        ...
        if filename:
            self.results_writer = ResultsWriter(
                filename, header={"t_start": self.t_start, "env_id": env_id}, extra_keys=info_keywords + ("i",)
            )
        else:
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        for i in range(len(dones)):
            if dones[i]:
                ...
                episode_info = {"i": i, "r": episode_return, "l": episode_length, "t": round(time.time() - self.t_start, 6)}
