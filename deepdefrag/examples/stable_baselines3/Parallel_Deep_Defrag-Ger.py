...
if __name__ == '__main__':
 ...
 for topology_name, load in zip(topologies, loads):
     ...
     ent_coef = 0.2
     lists = []
     for penalty_cycle, penalty_movement, algorithm  in [(-0.8, -0.1, 1) ]:
                             ...
                             for episode_length in [400]:
                                 for only_FF in [True]:
                                     ...
                                     env_args = dict(topology=topology, seed=10, load=load, num_spectrum_resources=320,
                                                     ...
                                                     only_FF = only_FF)
                                     log_dir = f"./results3/DeepDefrag-{topology_name}-{load}-{penalty_cycle}-{penalty_movement}-{number_options}-{seed}-" \
                                         f"{algorithm}-{number_layer}-{ent_coef}-{number_neorons}-{discount_factor}-{learning_rate}-{only_FF}-{episode_length}-v2/"
